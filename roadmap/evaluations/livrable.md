...
| GithubProject | <p>Nous offre l'état du projet en utilisant la structure Kaban suivante:<br>* Todo : <em>feature</em> non réalisées, non terminées<br>* In Progress : <em>features</em> en cours de réalisation<br>* In Review : <em>features</em> prêtes pour la review<br>* Done: <em>features</em> validées par l'enseignant - elles pourront être intégrés à la prochaine <em>release</em></p>                                           |
| Livrable      | <p>Une issue est assignée à l'enseignant mentionnant:<br>* Le dernier commit sur la branch <em>develop</em> ainsi que sur les éventuelles branches<br>* Un lien pointant sur une vidéo présentant le Kaban</p>                                                                                                                                                                                                               |
...
D'un point de vue fonctionnalité, l'idée est d'avoir un composant important facilitant le travail des micro-services de manière asynchrone, déposant et livrant les résultats sur un _bucket_.
...
Note1 : La différence entre un _bucket_ et un \*object" ne doit pas être à la charge de l'utilisateur.

Note2 : Optez une approche permettant à votre composant de dialoguer avec plusieurs _buckets_.

#### Contraintes techniques

Voici les composants minimual à développer, ainsi que les contraintes à respecter:
...
* \[Service]
  * \[] Ne contient aucune traces d'un provider spécifique
* \[Design Pattern]
...
* \[Test]
  ...
  * \[] IoC/DI - L'injection de dépendance est implémenté pour permettre le test unitaire de l'implémentation de l'adapter
...
La solution offre un déploiement "classique" tout comme via Docker. Pensez-y pour le dév, pour les githubactions ainsi que le déploiement.
...
* Image de prod
  * \[] ne contient que des dépendances de prod (pas de jdk)
...
* Conteneur
  * \[] aucune vulnerabilités de type "high" sur l'image de prod
  * \[] seul le port indispensable est publié
* Code
  * \[] aucune vulnerabilités de type "hight" sur les dépendances du projet
